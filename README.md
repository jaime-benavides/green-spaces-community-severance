...
Step 1. 01_prep_ses.R - ACS tract-level socioeconomic indicators and Index of Concentration at the Extremes (ICE)
...
Step 2. 02_prep_csi.R - population-weighted Community Severance Index (CSI) aggregation to census tract

- community_severance_nyc_census_tract.rds
- community_severance_la_census_tract.rds

Step 3. 03_prep_greenspace.R - NDVI and Euclidean distance to nearest public green space

- ndvi_nyc_census_tract.rds, ndvi_la_census_tract.rds
- cs_access_euclidean_nyc.rds, cs_access_euclidean_la.rds

Step 4. 04_prep_building_density.R - tract-level building density

- building_dens_nyc.rds, building_dens_la.rds

Step 5b. 05b_prep_cbg_nh_combined.R - merges primary and supplementary Advan CBG-level neighboring-home (NH) files

- 2019_full_year_neighbor_home_nyc_la_cbg_combined.csv - single canonical CBG-level NH input for all downstream processing

Step 6. 06_prep_neighbor_visits_annual_average.R - aggregates NH visits to tract-level annual averages; restricts to green-space CBGs
...
Step 5. 05_models_linear.R - NDVI (Gaussian GAM) and proximity (Gamma GAM) models
...
Step 7. 07_models_neighbor_visits_annual_average.R - neighboring-home visits GAMs (negative binomial)
...
06_07_run_neighbor_visits_workflow.R - orchestrates 06_prep_neighbor_visits_annual_average.R + 07_models_neighbor_visits_annual_average.R (Steps 6-7)

### Figures and tables list:

Step 7b. 07b_generate_nh_ice_q1_q5_figure.R - NH ICE Q1/Q5 stratified figure

- models_result_neighbor_visit_q1_q5_ICE_inc_2019_full_year.png - NH ICE model object source (combined into Fig 4)

Step 7c. 07c_generate_linear_ice_outl_figures.R - NDVI/proximity ICE Q1/Q5 and outlier-excluded figures (primary manuscript figures)
...
Step 7e. 07e_regenerate_manuscript_figures.R - regenerates all manuscript smooth figures from saved model objects (no re-fitting), including Figure 4 and sensitivity Figures S3/S4

Step 7d. 07d_generate_figure1_maps.R - Figure 1 and supplementary spatial maps

- figure1_nh_csi_maps.png - Figure 1
- supp_map_ice_inc.png - Figure S1, panel (a)
- supp_map_ice_q1_q5.png - Figure S1, panel (b)

Step 8a. 08a_table1_outcome_descriptives_neighbor_visits.R - Table 1 (outcomes, exposure & covariates)

- table1_descriptives_2019_full_year.tex

Step 8b2. 08b2_generate_supp_table_nh_missingness.R - Supplementary Table S1 (missing vs. analytic sample); reads `nh_exclusion_reason_diagnosis.csv` from `08b1_diagnose_nh_exclusion_reason.R` (Step 8b1, listed under Diagnostics below)

- supp_table_nh_missingness.tex

Step 8c. 08c_extract_numeric_results.R - Q25-to-Q75 quartile contrasts, the source of Table S2 and Results-text numbers

- numeric_results_quartile_contrasts.csv

Step 8c2. 08c2_generate_supp_table_s2_per_iqr.R - Supplementary Table S2, generated from numeric_results_quartile_contrasts.csv

Step 8d. 08d_extract_outlier_exclusion_counts.R - "excluded tracts" numbers cited in-text

Step 8e. 08e_extract_tract_area_by_city.R - tract area by city, context for the LA-vs-NYC distance comparison

Step 8f. 08f_generate_supp_table_outlier_geography.R - Supplementary Table S3 (outlier-tract geography and built environment)

Step 8g. 08g_generate_nh_distribution_figure.R - NH visits distribution figure (Fig S2a)

Step 8h. 08h_extract_ice_nh_quartile_contrasts.R, 08h_extract_ice_ndvi_quartile_contrasts.R, 08h_extract_ice_distance_quartile_contrasts.R - ICE-stratified (Q1/Q5) quartile contrasts cited in §3.5

Step 8i. 08i_extract_ice_effect_modification_contrasts.R - ICE effect-modification statistics cited in §3.5

Step 8j. 08j_extract_ice_effect_modification_difference_contrasts.R - reads the three Step 8h CSVs to compute Q5-vs-Q1 difference contrasts, cited in §3.5

Step 8k. 08k_extract_manuscript_misc_counts.R - missingness/sample-size counts cited in-text; reads Step 8d's output
...
Step 8b1. 08b1_diagnose_nh_exclusion_reason.R - diagnoses NH exclusion reasons; **its output is a required input to Step 8b2** (`08b2_generate_supp_table_nh_missingness.R`, Supplementary Table S1), so despite being framed as a diagnostic script it is part of the manuscript pipeline

Step 8l. 08l_inspect_table2_missingness.R - reproduces the check behind the Table 2 missingness footnote; **its output (`table2_missingness_diagnosis.csv`) is checked by Step 9's audit**, so despite being framed as a diagnostic script it is part of the manuscript pipeline

## Data (data) list:

### Self-generated (copied into data/raw/, description - file name - provenance):

CSI factor scores (LA) - data/raw/csi/csi_scores_la.rds - community_severance_us PCP + factor-analysis pipeline

CSI factor scores (NYC) - data/raw/csi/csi_scores_nyc.rds - community_severance_nyc PCP + factor-analysis pipeline

Pre-processed ACS estimates table - data/raw/acs/acs_dt.rds - community_severance_nys_climate_change_mh pipeline
