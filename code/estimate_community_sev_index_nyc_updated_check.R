...
dta_inputs <- readRDS("/Volumes/Extreme SSD/laptop_back_up/maklab/scratch/workspace/community_severance_us/data/generated/community_severance_nyc_input_data_unscaled.rds")

# standardize

# homogenize data ranges by scaling by standard deviation
dta_inputs_id <- dta_inputs[,c("GEOID20")]
dta <- dta_inputs[ , -c(which(colnames(dta_inputs) == "GEOID20"))]
dta_scaled = as.data.frame(apply(dta, 2, function(a) a/sd(a, na.rm = T)))
dta_prep <- cbind(dta_inputs_id, dta_scaled)
...
data_desc <- readRDS(paste0("/Volumes/Extreme SSD/laptop_back_up/maklab/scratch/workspace/community_severance_us/data/generated/", "smart_location_data_subset_desc.rds"))
...
saveRDS(pcp_outs, file = paste0(generated.data.folder, "pcp_rrmc_", name_sim, "_updated_check.rds"))
...
png(paste0(output.folder, "pcp_rrmc", "_l_matrix_correlations_", name_sim, "_updated_check.png"), 900, 460)
...
png(paste0(output.folder, "raw_mat_corr_", name_sim, "_updated_check.png"), 900, 460)
...
png(paste0(output.folder, "_l_fa_", p, "_patterns_", name_sim, "_updated_check.png"), 1250, 460)
...
saveRDS(dat_scores, paste0(generated.data.folder, "csi_scores_nyc_check.rds"))
normalize <- function(x) {
  return ((x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - min(x, na.rm = TRUE)))
}
dat_scores$MR1_norm <- normalize(dat_scores$MR1)
saveRDS(dat_scores, paste0(generated.data.folder, "comm_sev_fa_scores", name_sim, "_dta_us_updated_check.rds"))
